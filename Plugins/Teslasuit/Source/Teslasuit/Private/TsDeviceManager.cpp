...
{
    if (!ITeslasuitPlugin::Get().IsInitialized())
    {
        UE_LOG(LogTemp, Error, TEXT("UTsDeviceManager: Core was not initialized"));
        return;
    }
...
void UTsDeviceManager::UnInitialize()
{
    ...
    if (!ITeslasuitPlugin::Get().IsInitialized())
    {
        UE_LOG(LogTemp, Log, TEXT("UTsDeviceManager: Core was not initialized"));
        return;
    }
    auto& Provider = ITeslasuitPlugin::Get().GetDeviceProvider();
